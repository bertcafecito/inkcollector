...
class TestErrorScenarios:
    ...
    def test_json_dump_error_in_save_sets(self, cli_with_temp_dirs):
        ...
        mock_sets = [{"id": "set1", "invalid_data": object()}]  # Non-serializable object
        ...
        with patch('builtins.open', mock_open()), \
            ...
            error_calls = [call for call in mock_print.call_args_list if "Error saving" in str(call)]
            assert len(error_calls) > 0

    def test_json_dump_error_in_save_cards(self, cli_with_temp_dirs):
        """Test JSON serialization error when saving cards."""
        mock_cards = [{"id": "card1", "invalid_data": object()}]  # Non-serializable object
        set_id = "test-set"
    ...
    def test_card_image_download_partial_failure(self, cli_with_temp_dirs):
        ...
        mock_cards = [
            {
                "id": "card1",
                "image_uris": {
                    "digital": {"normal": "http://example.com/card1.jpg"}
                }
            },
            {
                "id": "card2",
                "image_uris": {
                    "digital": {"normal": "http://example.com/card2.jpg"}
                }
            },
        ...
        with patch.object(cli_with_temp_dirs, '_create_directory_if_not_exists'), \
             patch('builtins.print') as mock_print:
            
            cli_with_temp_dirs._download_card_images(mock_lorcast, mock_cards, "set1")
            
            # Should report 1 successful download out of 3 cards
            mock_print.assert_any_call("Successfully downloaded 1 out of 3 card images.")
    ...
    def test_unicode_handling_in_file_operations(self, cli_with_temp_dirs):
        ...
        mock_sets = [
            {"id": "set1", "name": "Tëst Sét wïth Ünïcödë", "description": "Descripción en español"}
        ]
...
class TestEdgeCases:
    ...
    def test_very_large_dataset_handling(self):
        ...
        with patch('builtins.print'), \
             patch('json.dumps', return_value="mock_json"):

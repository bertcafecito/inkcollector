...
class TestInkcollectorCLI:
    ...
    def test_create_directory_if_not_exists_creates_new_directory(self, mock_exists, mock_makedirs, cli):
    ...
    @patch('os.makedirs')
    @patch('os.path.exists')
    def test_create_directory_if_not_exists_skips_existing_directory(self, mock_exists, mock_makedirs, cli):
    ...
    def test_parser_lorcast_get_cards_arguments(self, cli):
        ...
        args = cli.parser.parse_args(['lorcast', 'get-cards', '--set-id', 'test-set', '--get-images'])
        assert args.get_images == 'normal'  # Default size

        args = cli.parser.parse_args(['lorcast', 'get-cards', '--set-id', 'test-set', '--get-images', 'large'])
    ...
    def test_handle_lorcast_command_unknown_subcommand(self, mock_lorcast_class, cli):
        ...
        args = argparse.Namespace(
            command='lorcast',
            lorcast_command='unknown-command'
        )
        
        with patch('builtins.print') as mock_print:
            cli.handle_lorcast_command(args)
            
        mock_print.assert_called_once_with("Unknown lorcast subcommand: unknown-command")
    ...
    def test_handle_get_cards_command_with_images(self, cli, mock_lorcast_api):
        ...
        mock_download.assert_called_once_with(mock_lorcast_api, mock_cards, 'set1', 'large')
    ...
    def test_save_sets_to_file_success(self, mock_json_dump, mock_file, cli):
        ...
        with patch.object(cli, '_create_directory_if_not_exists') as mock_create_dir, \
            ...
            mock_json_dump.assert_called_once_with(mock_sets, mock_file.return_value.__enter__.return_value, ensure_ascii=False, indent=2)
    ...
    def test_save_sets_to_file_error(self, mock_file, cli):
        ...
        with patch.object(cli, '_create_directory_if_not_exists'), \
             patch('builtins.print') as mock_print:
    ...
    def test_save_cards_to_file_success(self, mock_json_dump, mock_file, cli):
        ...
        with patch.object(cli, '_create_directory_if_not_exists') as mock_create_dir, \
            ...
            mock_create_dir.assert_called_once()
            mock_file.assert_called_once()
            mock_json_dump.assert_called_once_with(mock_cards, mock_file.return_value.__enter__.return_value, ensure_ascii=False, indent=2)
    ...
    def test_download_card_images(self, cli, mock_lorcast_api):
        ...
        with patch.object(cli, '_create_directory_if_not_exists'), \
             patch.object(cli, '_download_single_card_image', return_value=True) as mock_download_single, \
             patch('builtins.print') as mock_print:
    ...
    def test_download_single_card_image_success(self, cli, mock_lorcast_api):
        ...
        result = cli._download_single_card_image(mock_lorcast_api, mock_card, output_path)
        
        assert result is True
        mock_lorcast_api.download_image.assert_called_once_with(
            "http://example.com/image.jpg",
            os.path.join(output_path, "crd_card1.jpg")
        )
    ...
    def test_download_single_card_image_no_id(self, cli, mock_lorcast_api):
        ...
        with patch('builtins.print') as mock_print:
            result = cli._download_single_card_image(mock_lorcast_api, mock_card, "/test/path")
            
        assert result is False
        mock_print.assert_called_once_with("Card ID not found, skipping image download.")
    ...
    def test_download_single_card_image_no_image_uris(self, cli, mock_lorcast_api):
        ...
        assert result is False
        mock_print.assert_called_once_with("No image URIs found for card card1, skipping image download.")
    ...
    def test_download_single_card_image_download_error(self, cli, mock_lorcast_api):
        ...
        assert result is False
        mock_print.assert_called_once_with("Error downloading image for card card1: Download failed")

...
class TestArgumentParsing:
    ...
    def test_help_message_main(self, cli):
        ...
        assert 'CLI tool for collecting data about the disney lorcana trading card game' in help_text
    ...
    def test_get_cards_all_flag_combinations(self, cli):
        ...
        # With JSON output
        args = cli.parser.parse_args(['lorcast', 'get-cards', '--set-id', 'test', '--json'])
        assert args.json

        # With save JSON
        args = cli.parser.parse_args(['lorcast', 'get-cards', '--set-id', 'test', '--save-json'])
        assert args.save_json

        # With image download (default size)
        args = cli.parser.parse_args(['lorcast', 'get-cards', '--set-id', 'test', '--get-images'])
        assert args.get_images == 'normal'

        # With image download (specific size)
        for size in ['small', 'normal', 'large']:
            args = cli.parser.parse_args(['lorcast', 'get-cards', '--set-id', 'test', '--get-images', size])
            assert args.get_images == size

        # All flags together
        args = cli.parser.parse_args([
            'lorcast', 'get-cards', '--set-id', 'test',
    ...
    def test_argument_order_independence(self, cli):
        ...
        orders = [
            ['lorcast', 'get-cards', '--set-id', 'test', '--json', '--save-json', '--get-images', 'large'],
            ['lorcast', 'get-cards', '--json', '--set-id', 'test', '--get-images', 'large', '--save-json'],
            ['lorcast', 'get-cards', '--get-images', 'large', '--save-json', '--json', '--set-id', 'test']
        ]
    ...
    def test_case_sensitivity(self, cli):
        ...
        args = cli.parser.parse_args(['lorcast', 'get-cards', '--set-id', 'MiXeD_CaSe_SeT'])
...
class TestCommandLineInterface:
    ...
    def test_help_formatting(self, cli):
        ...
        assert 'positional arguments:' in help_text.lower() or 'commands:' in help_text.lower()
        assert 'optional arguments:' in help_text.lower() or 'options:' in help_text.lower()
...
class TestArgumentValidation:
    ...
    def test_argument_parsing_with_equals_sign(self, cli):
        ...
        try:
            args = cli.parser.parse_args(['lorcast', 'get-cards', '--set-id', 'test', '--get-images=large'])
            assert args.get_images == 'large'
        except SystemExit:
            # If equals syntax isn't supported, that's also valid
            pass

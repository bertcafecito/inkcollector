...
class InkcollectorCLI:
    ...
    def _handle_get_sets_command(self, lorcast: LorcastAPI, args: argparse.Namespace) -> None:
    ...
    def _handle_get_cards_command(self, lorcast: LorcastAPI, args: argparse.Namespace) -> None:
        ...
        if args.get_images:
            self._download_card_images(lorcast, cards, set_id)
    ...
    def _save_cards_to_file(self, cards: List[Dict[str, Any]], set_id: str) -> None:
        ...
        Args:
            ...
            set_id: ID of the set
        """
        output_path = os.path.join(self.data_output_dir, self.LORCAST_DATASOURCE_DIR, "sets")
        self._create_directory_if_not_exists(output_path)
    ...
    def _download_card_images(self, lorcast: LorcastAPI, cards: List[Dict[str, Any]], set_id: str) -> None:
        ...
        Args:
            ...
            set_id: ID of the set
        """
        output_path = os.path.join(self.image_output_dir, self.LORCAST_DATASOURCE_DIR, "sets", set_id)
        self._create_directory_if_not_exists(output_path)
        ...
        for card in cards:
            if self._download_single_card_image(lorcast, card, output_path):
                successful_downloads += 1
        
        print(f"Successfully downloaded {successful_downloads} out of {len(cards)} card images.")
    
    def _download_single_card_image(self, lorcast: LorcastAPI, card: Dict[str, Any], output_path: str) -> bool:
        ...
        if not digital_uris:
            print(f"No digital image URIs found for card {card_id}, skipping image download.")
            return False
        
        image_uri = digital_uris.get('large')
        if not image_uri:
